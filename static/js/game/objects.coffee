...
class window.Brew.Item extends Brew.Thing
	constructor: (item_info) ->
		...
		# handle special attributes
		if not item_info.group?
			console.error("bad item with no group")
...
# inventory_key_list = (String.fromCharCode(x) for x in [49..57])

class window.Brew.Inventory
	# contains a bunch of items
	constructor: (max_items) ->
		@items = {}
		@max_items = max_items ? Brew.config.max_inventory_items
	...
	addItem: (new_item, inv_key_override) ->
		if (@getItems().length == @max_items) and (not inv_key_override?)
			return null
		else
			next_key = inv_key_override ? @getNextKey()
...
class window.Brew.Monster extends Brew.Thing
	constructor: (monster_info) ->
		...
		@memory = {}
		@pathmaps = {}
		@sight_radius = monster_info.sight_radius ? Brew.config.default_sight_radius
	...
	# FIELD OF VIEW
	canView: (xy) ->
		if @hasFlag(Brew.flags.see_all.id)
			return true
		else
	...
	updateKnowledgeAt: (level, xy) ->
		...
		if mob? and not mob.hasFlag(Brew.flags.is_invisible.id) # what about can see invisible?
...
class window.Brew.Level
	constructor: (@depth, @width, @height, options) ->
		...
		@ambient_light = options?.ambient_light ? [153, 153, 153] #[130, 130, 130]
		# @walkable_xy_list = []
		# @walkable_key_list = []
		@navigation = {}
	...
	updateLightMap: () ->
		...
		# ok lets figure out what changed
		changes = []
		for own key, color of @light
			# check old light level
			prev_color = old_light[key]
			if not prev_color?
				changes.push(xy)
				
			if prev_color?
				delete old_light[key]

				if not Brew.utils.compareColor(prev_color, color)
					# console.log(prev_color, color, xy)
					# if @checkValid(xy)
					changes.push(xy)

		# now go through remaining old lights (since they are gone, they changed)
		for own key, val of old_light
			xy = keyToCoord(key)
			# if @checkValid(xy)
			changes.push(keyToCoord(key))
