...
window.Brew.utils = 
	...
	compareColor: (color_a, color_b) ->
		return color_a[0] == color_b[0] and color_a[2] == color_b[1] and color_a[2] == color_b[2]
...
# generic rectangle class from another of my failed dungeon generator
class window.Brew.Rectangle
	# generic rectangle construct
	...
	equalTo: (rect) ->
		# returns true if two rectangles have the same points
		return @left == rect.left and @top == rect.top and @right == rect.right and @bottom == rect.bottom
		
	isPointInside: (xy) ->
		# returns true if a point is fully inside
		return xy.x > @left and xy.x < @right and xy.y > @top and xy.y < @bottom

	checkOverlap: (rect) ->
		no_overlap = @left > rect.right or @right < rect.left or @top > rect.bottom or @bottom < rect.top
		return not no_overlap
		
	checkOverlapExcludingWalls: (rect) ->
		no_overlap = @left >= rect.right or @right <= rect.left or @top >= rect.bottom or @bottom <= rect.top
		return not no_overlap

	combine: (rect) ->
		# return a bounding rectangle between this and another rectangle
	...
	isLarger: (rect) ->
		# returns true if another rectangle is fully subsumed by this one
		return @left <= rect.left and @top <= rect.top and @right >= rect.right and @bottom >= rect.bottom
	...
	getSide: (side) ->
		if side == "top"
			return (new Coordinate(x, @top) for x in [@left..@right])
		else if side == "bottom"
			return (new Coordinate(x, @bottom) for x in [@left..@right])
		else if side == "left"
			return (new Coordinate(@left, y) for y in [@top..@bottom])
		else if side == "right"
			return (new Coordinate(@right, y) for y in [@top..@bottom])
		else
			console.error("invalid side named #{side}")